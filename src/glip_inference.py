...
def load_model_glip(config_file, weight_file):
    cfg.local_rank = 0
    cfg.num_gpus = 1
...
def glip_inference(glip_demo, save_dir, part_names, num_views=10,
    ...
    pred_dir = os.path.join(save_dir, "glip_output")
    os.makedirs(pred_dir, exist_ok = True)
    predictions = []
    for i in range(num_views):
        image = load_img("%s/%d.png" % (save_dir, i))
...
def glip_inference_noLoad(glip_demo, images, part_names, num_views=10):
